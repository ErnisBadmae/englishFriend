...
- Compare one external voice provider at a time.

## Task Tracking Rules

For Codex and Claude Code:

- Read `!DOC/operations/CURRENT_PRODUCT_STATE.md` first.
- Read this file before planning product work.
- Do not start voice provider implementation unless the user explicitly asks or the voice gate is marked open.
- Prefer product-loop fixes over infrastructure polish.
- If a change affects product behavior, update `CURRENT_PRODUCT_STATE.md` briefly.
- If a change affects the workplan, update this document and `!DOC/strategy/ROADMAP.md` links/status.
- Keep long session narratives out of `CURRENT_PRODUCT_STATE.md`.

## Decision Rules

When choosing between two tasks:

- choose text loop over voice polish
- choose evidence continuity over UI polish
- choose interview specificity over broad English coverage
- choose real pilot signal over architecture elegance
- choose a small eval-backed fix over a broad refactor
- choose observability only when it helps diagnose a pilot or eval failure

## Anti-Roadmap For This Cycle

Do not work on:

- generic tutor mode
- vocabulary drill productization
- grammar rescue as a standalone path
- broad `workplace_communication`
- broad `project_walkthrough`
- voice provider integration as a mainline task
- provider leaderboards detached from our scenarios
- graph/vector expansion unless it directly fixes mission/evidence continuity
- new UI surfaces that do not support session completion or session 2 return

## Voice Re-Evaluation Trigger

Voice returns to active planning only if at least one condition is true:

- a pilot explicitly says speaking pressure is the missing value after a useful text session
- text-first session 2 works, but engagement is limited by lack of spoken practice
- current voice baseline blocks a user who otherwise finds the product valuable
- we need a provider comparison to decide a concrete premium path

Even then, the next step is benchmark design, not direct provider migration.

## Reference Documents

- [../operations/CURRENT_PRODUCT_STATE.md](../operations/CURRENT_PRODUCT_STATE.md)
- [ROADMAP.md](./ROADMAP.md)
- [../operations/PRODUCT_SYNTHETIC_EVAL_RUNBOOK.md](../operations/PRODUCT_SYNTHETIC_EVAL_RUNBOOK.md)
- [../operations/STT_BENCHMARK_RUNBOOK.md](../operations/STT_BENCHMARK_RUNBOOK.md)
- [../operations/VOICE_OBSERVABILITY_RUNBOOK.md](../operations/VOICE_OBSERVABILITY_RUNBOOK.md)
- [../operations/DISCOVERY_PHASE0_RUNBOOK.md](../operations/DISCOVERY_PHASE0_RUNBOOK.md)
