...
1. Пользователь запускает Telegram апку
   ↓
2. Приветственное сообщение + краткое описание возможностей
   ↓
3. Определение уровня английского:
   - Быстрый тест (A1-C2) или самооценка
   - Сохранение в `users.language_level`
   Опционально: предполагаю что лучше в раздел профиля убрать
   ↓
4. Сбор интересов:
   - Выбор из популярных категорий (IT, путешествия, etc.)
   - Возможность ввести свои интересы
   - Сохранение в `user_interests`
     Опционально: предполагаю что лучше в раздел профиля убрать
   ↓
5. Короткий вводный диалог (2-3 минуты)
   - Тест голосового взаимодействия

Этап 2: Регулярное использование (сессия 10-15 минут)
1. Пользователь запускает звонок через Mini App
   ↓
2. WebRTC установление соединения
   ↓
3. Начало сессии с персональным приветствием:
   - Агент вспоминает прошлые темы
   - "Привет! Как продвигается твой проект на Python?"
   ↓
4. Интерактивный диалог в реальном времени:
   ...
   ↓
5. Естественное завершение по таймеру(посчитать экономику токенов) или инициативе пользователя
   ↓
6. Автоматический пост-анализ:
...
text
1. Автоматическое сообщение в Telegram чат:
   "Отличная работа! Вот твой прогресс за сегодня:"
   ↓
